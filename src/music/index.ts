// Strautomator Core: Music

import {MusicTrack, TrackLyrics} from "./types"
...
/**
 * Music lyrics fetcher (Genius, STANDS4) with database-backed caching.
 */
export class Music {
    private constructor() {}
    ...
    /**
     * Init the Music wrapper.
     */
    init = async (): Promise<void> => {
        try {
            cache.setup("lyrics", settings.music.cacheDuration)
        } catch (ex) {
            logger.error("Music.init", ex)
            throw ex
        }
    }
     ...
     */
    private fromGenius = async (track: MusicTrack): Promise<string> => {
        if (!settings.genius.api.token) {
            logger.warn("Music.fromGenius", logHelper.musicTrack(track), "Missing settings.genius.api.token, won't fetch")
            return null
        }

        try {
            logger.debug("Music.fromGenius", logHelper.musicTrack(track), "Fetching lyrics from Genius")
            ...
            logger.error("Music.fromGenius", logHelper.musicTrack(track), ex)
            throw ex
        }
    }
     ...
     * @param track Music track.
     */
    private fromStands4 = async (track: MusicTrack): Promise<string> => {
        if (!settings.stands4.api.token) {
            logger.warn("Music.fromStands4", logHelper.musicTrack(track), "Missing settings.stands4.api.token, won't fetch")
            return null
        }

        try {
            logger.debug("Music.fromStands4", logHelper.musicTrack(track), "Fetching lyrics from STANDS4")
            ...
            logger.error("Music.fromStands4", logHelper.musicTrack(track), ex)
            throw ex
        }
    ...
    /**
     * Get the lyrics for the specified music track. Never throws, will return
     * null if not found or if an error occurs.
     * @param track Music track.
     */
    getLyrics = async (track: MusicTrack): Promise<string> => {
        try {
            const cacheId = `lyrics-${track.id}`
            const cached: string = cache.get("lyrics", cacheId)
            if (cached) {
                logger.info("Music.getLyrics.fromCache", logHelper.musicTrack(track), "From memory")
                return cached
            }
            ...
            if (dbCached) {
                logger.info("Music.getLyrics.fromCache", logHelper.musicTrack(track), "From database")
                return dbCached.lyrics
            }

            // First try the STANDS4 API.
            // Nothing found? Try again with Genius.
            let lyrics = await this.fromStands4(track)
            if (!lyrics) {
                lyrics = await this.fromGenius(track)
            }

            // No lyrics found.
            if (!lyrics) {
                logger.info("Music.getLyrics", logHelper.musicTrack(track), "Lyrics not found")
                cache.set("lyrics", cacheId, null)
                return null
            }
            ...
            const dbEntry: TrackLyrics = {
                id: track.id,
                lyrics: lyrics,
                dateExpiry: dayjs().add(settings.music.maxCacheDuration, "seconds").toDate()
            }

            await database.set("lyrics", dbEntry, track.id)
            cache.set("lyrics", cacheId, lyrics)
            logger.info("Music.getLyrics", logHelper.musicTrack(track), `Length: ${lyrics.length}`)

            return lyrics
        } catch (ex) {
            logger.error("Music.getLyrics", logHelper.musicTrack(track), ex)
            return null
        }
    }
}
